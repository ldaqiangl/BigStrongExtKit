...
@implementation BSDatabaseOperationItem
...
@interface BSDatabaseOperation ()
@property (strong, nonatomic) FMDatabaseQueue * dbQueue;
@end
@implementation BSDatabaseOperation
...
- (void)createTableWithName:(NSString *)tableName {
    if ([BSDatabaseOperation checkTableName:tableName] == NO) {
        return;
    }
...
- (id)getObjectById:(NSString *)objectId fromTable:(NSString *)tableName {
    BSDatabaseOperationItem * item = [self getBSDatabaseOperationItemById:objectId fromTable:tableName];
    if (item) {
...
- (BSDatabaseOperationItem *)getBSDatabaseOperationItemById:(NSString *)objectId fromTable:(NSString *)tableName {
    ...
    if (json) {
        ...
        BSDatabaseOperationItem * item = [[BSDatabaseOperationItem alloc] init];
...
- (NSArray *)getAllItemsFromTable:(NSString *)tableName {
    ...
    NSError * error;
    for (BSDatabaseOperationItem * item in result) {
        error = nil;
